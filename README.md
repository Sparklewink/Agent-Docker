...
[![Docker Build](https://github.com/Sparklewink/agent/actions/workflows/docker-build.yml/badge.svg)](https://github.com/Sparklewink/agent/actions/workflows/docker-build.yml)
...
```bash
ghcr.io/sparklewink/agent:latest
```
...
3.  在镜像地址输入框中，完整粘贴上面的镜像地址 `ghcr.io/sparklewink/agent:latest`。
4.  根据上面的表格，在平台的环境变量设置中，添加探针密钥 (`NZ_SERVER`, `NZ_CLIENT_SECRET` 等)。
5.  选择免费套餐并点击 **“部署”**。

## ⚠️ 注意事项

- **镜像可见性**: 本仓库的 Docker 镜像 (`ghcr.io/sparklewink/agent`) 必须在 GitHub Packages 中设置为 **"Public" (公开)**，外部平台才能成功拉取。
